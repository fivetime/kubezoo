...
## AJ. 租户能不能自己跑 ingress 控制器?—— 引用与对象名对不上(实测)

**产品问题**(用户提出):租户想在自己命名空间里起 nginx/haproxy 做**内部** LB,
该不该给这个权限?设想的契约是:**`ingressClassName` 指向平台的(Octavia 那个)才接公网,
其余只走内部**。

流量路径决定了这个契约是对的:外部 → Octavia(kubetron)→ **平台的** ingress 控制器 → 租户 Service。
租户不掌握基础设施,它自己那个控制器天然只能是内部的。

### ⛔ 但今天做不到,而且方向是反的

租户建一个 IngressClass `internal` 并让自己的 Ingress 引用它:

| | 租户视角 | 上游实际 |
|---|---|---|
| IngressClass **对象** | `internal` | **`111111-internal`**(集群级资源,名字加前缀) |
| Ingress 里的**引用** | `internal` | **`internal`** —— **没加前缀** |

`pkg/convert` 里**一处都没碰过 `ingressClassName`**(与 `runtimeClassName` /
`priorityClassName` 同一类,#82 finding I 记过)。于是:

1. **租户自己的控制器永远选不到自己的 Ingress** —— 它监听 `111111-internal`,
   而 Ingress 里写的是 `internal`
2. ⚠️ **更要紧的是反过来**:那个不带前缀的引用是**平台命名空间里的名字**。
   租户写 `nginx` 就直接命中**平台的公网控制器** —— 这才是暴露路径

⇒ 现在的"擦掉 class"策略其实是在**挡住第 2 条**,代价是第 1 条也一起没了
(租户的 Ingress 成摆设,§AH)。

### ⚠️ 我差点做反:无条件替换成平台 class 是更糟的

一度把策略从"擦除"改成"替换成平台 class"。**那等于把租户的每一个 Ingress 都变成公网入口** ——
租户本想在内部起个 LB,结果被交给平台控制器暴露出去。**已撤回,没有提交。**

### ✅ 已实现

**把 `spec.ingressClassName` 当成集群级对象引用来改写** —— 写时加前缀、读时去前缀,
和 namespace 一样;**只放行一个白名单里的平台 class 不加前缀**。

```
租户写 internal   → 上游 111111-internal → 命中租户自己的 IngressClass  ✅ 内部 LB
租户写 <平台class> → 不加前缀            → 命中平台控制器              ✅ 公网,且是显式请求
租户写 nginx(平台的别的 class,不在白名单) → 111111-nginx → 谁都命不中  ✅ 失败关闭
```

按 §8.0 判据这属于 **kubezoo**(集群级对象引用 + 双向改写),**不是策略层** ——
策略层那条擦除规则**已删除**。
"指向 Octavia 才接公网"因此从**约定**变成了**机制**。

实现:`pkg/convert/ingress.go`(与 PVC 的 `spec.volumeName` 同一形状),
白名单来自新 flag `--public-ingress-classes`(**默认空 ⇒ 租户的 Ingress 全是内部的**)。
⚠️ 废弃的 `kubernetes.io/ingress.class` 注解**一起改写** —— 多数控制器仍认它,
而且它**压过**字段,只改字段等于留了后门。

**实测(lab 里装真的 ingress-nginx,`--public-ingress-classes=nginx`)**:

| 租户写 | 上游存 | 平台控制器 |
|---|---|---|
| `internal`(自己的) | `111111-internal` | ⛔ 未接管 ✅ |
| `nginx`(白名单) | `nginx` | ✅ **接管**,归 `111111-default/ing-nginx` |
| `222222-nginx`(别人的) | `111111-222222-nginx` | ⛔ 未接管 ✅ |

三条**读回来都与写入一致**(否则租户重新 apply 自己的清单会被二次前缀)。

守卫 `verify.sh` 四条,**已验证摘掉转换器会红**(自己的类不前缀化 / 借用的不被收敛,
两条恰好红;白名单与往返那两条不受影响 —— 说明断言分得开)。
...
## AM. 主机名裁决 ✅ 已实现(默认安全的那一半)

§AH 实测的问题:`host` 是自由 DNS 名,kubezoo 表达不了(前缀化一个域名就毁了它的用途),
于是任何租户可声明任何主机名,由控制器按**创建顺序**裁决,**落败方零报错**。
抢不走已占用的,但**可以抢先占用**别人还没声明的 —— 那就是拒绝服务 + 接走本该属于别人的流量。

### 修法:每租户一个子域,域外一律拒

`config/policy/tenant-ingress-hostnames.yaml`,**原生 VAP**(一条 CEL 就够,
进程内、无 webhook、不吃 Kyverno 那些静默失效的坑)。

租户 ID 从 namespace 前缀取 —— 那个前缀**租户伪造不了**(kubezoo 写入时强制,
且拒绝租户改 `kubezoo.io/tenant` 标签)。

按 §8.0 判据归**策略层**:纯写路径、不看响应、且"域名方案换个平台就变"。

### 实测六条

| | |
|---|---|
| 自己子域 `shop.111111.apps.example.com` | ✅ created |
| **别人的子域** `shop.222222.…` | ⛔ 拒 |
| 任意外部域名 `bank.example.com` | ⛔ 拒 |
| **空 host**(等于占下整个入口) | ⛔ 拒 |
| **事后 UPDATE 改成越界** | ⛔ 拒(否则先合法申请再改名) |
| 平台自己 namespace 里任意域名 | ✅ 不受约束 |
| 多条 rule 里只要一条越界 | ⛔ 整个拒 |

守卫 `verify.sh` 六条,**已验证摘掉策略恰好红四条**(该拒的四条全变成"被接受"),
而"自己子域可用"和"平台不受约束"仍绿 —— 说明断言分得开。

### ⚠️ 只解决了默认安全的一半

真实客户会要用**自己的域名**(`shop.customer.com`),那需要:

1. **每租户的自定义域名白名单**
2. **域名归属校验**(DNS TXT 之类),否则白名单本身就成了新的抢占面
3. ⭐ 白名单必须放在**租户改不动**的地方 —— 租户能给自己的 namespace 打任意标签/注解
   (`NamespaceTransformer` 只钉死 `kubezoo.io/tenant` 一个),放那里就是
   "判定条件建立在租户可控输入上"那个老坑。候选:Tenant CR(租户写不了)或 VAP 的 paramRef

这一半是产品决策,未做。

### ⚠️ 顺带:加这条策略时,我自己先前的测试固件被它拒了
...
§AR 的结论:投影是"每 namespace 一条 RoleBinding",而 **RoleBinding 永远授不了集群级资源**
⇒ 租户 ClusterRole 里集群级的那部分**被静默丢弃**:租户写了、绑了、**一个报错都没有**,
它的 operator 在运行时才挂。§AS 把这一块拆成三条,这一节做掉其中干净的那条。

### 判据:**规则里的 API 组是不是全都带租户前缀**

租户的 CRD 组是 `111111-cert-manager.io` —— **组名自身带租户**。
所以一条只涉及这些组的规则,**结构上就够不到别人**:不需要过滤,因为里面根本没有别人的东西。
⇒ 可以给一条**真的、集群级的** ClusterRoleBinding。

从每条投影记录派生:读它引用的 ClusterRole,**只留下 API 组全部是本租户的规则**,
生成 `kubezoo:crgroups:<租户ID>:<名字>` 的 ClusterRole + ClusterRoleBinding,subject 照抄。

### ⭐ 过滤必须"整条留或整条丢",不能拆

| 规则 | |
|---|---|
| `["111111-cert-manager.io"]` | ✅ 留 |
| `["222222-cert-manager.io"]` | ⛔ 丢(带前缀,但不是它的) |
| `["*"]` | ⛔ 丢 |
| `[""]`(core) | ⛔ 丢 |
| `["111111-x", ""]` **混写** | ⛔ **整条丢**,不能只留前一半 |
| `["111111x.example.com"]` | ⛔ 丢(只是**开头像**租户 ID,不是 `<ID>-`) |
| 只有 nonResourceURLs | ⛔ 丢 |

单测 9 条盯这张表。

### ⚠️ 必须有 informer,否则"装完 10 分钟不能用"

租户建 ClusterRoleBinding **不会**让控制器重新处理这个租户 ⇒ 派生对象要等下一次
tenant resync(**最长 10 分钟**)。实测:新建后 30 秒仍未出现。
加了一条**按投影标签选择**的 RoleBinding informer 之后:**建 ~2s 生效、删 ~2s 撤销**。

⚠️ 这条不能像投影那样在**请求路径**上做:派生的是**集群级**对象,而 kubezoo 在请求路径上
冒充的是租户,**租户在集群级什么都没有**。只能由控制器以自己的身份写。

### 实测

| | |
|---|---|
| SA `list` 自己组里的集群级 CR | ✅ **yes**(任何 RoleBinding 都给不了这个) |
| SA `list` **别人组**里的同一资源 | ⛔ no —— **靠名字,不靠过滤** |
| 同一条 ClusterRole 里还授了 `secrets` / CRD | ⛔ **一样都没跟过来** |
| 删掉 ClusterRoleBinding | ✅ ~2s 撤销 |
| 冻结租户 | 派生对象一并撤除(read-only 则收窄为只读动词) |

守卫 5 条,**红测红 1 条** —— 恰好是那条"能力"断言;
另外四条是**安全性对照**,没有这个特性时本来就该绿(它们防的是**实现错**,不是**没实现**)。

### ⛔ 没做的两条,以及为什么

**① 原生集群级资源的 `list`/`watch`**(CRD / 两种 webhookconfiguration / apiservices)。
RBAC 里 `resourceNames` 对 list 不生效(**list 请求没有名字**),所以只能靠 §AN 的路径绑定组
—— 那意味着**隔离从"RBAC 兜底"退成"kubezoo 过滤正确"**,而且暴露面从"每租户一个身份"
变成"每个工作负载"。**需要单独决策。**

**② `certificatesigningrequests` + `signers/approve`** —— ⭐ 建议**不做**。
核过 `clusterScopedRules()`:租户今天**完全没有**这两项权限,
所以这不是"把租户已有的能力传下去",而是**新开一项权限**;
而批准 CSR = **签发客户端证书**,那是身份层的东西。

⚠️ 另记:`apiservices` kubezoo **不提供** —— 给它授权是**惰性的**(经 kubezoo 404、
不经 kubezoo 组不被断言),而且**没有任何报错指向"这条规则没意义"**。
