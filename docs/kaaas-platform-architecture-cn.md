...
### 3.0 ⭐ operator 能跑起来:集群级授权的两条路

"接近真实的 k8s 集群"这句话,**卡在集群级 RBAC 上**。租户的 ClusterRoleBinding 被投影成
**每个 namespace 一条 RoleBinding**,而 **RoleBinding 永远授不了集群级资源** ——
于是带集群级需求的 operator(cert-manager、Kyverno、external-secrets…)**装得上、跑不起来**,
而且租户侧看不到任何错误:它写的 binding 被接受了。

⇒ 两条路,**判据不同、机制不同、安全论证也不同**:

| | 覆盖什么 | 怎么授 | 为什么安全 |
|---|---|---|---|
| **A** | 租户**自己前缀的组**(ClusterIssuer / ClusterPolicy 这类它自己的 CR) | 一条**真的** ClusterRoleBinding | ⭐**构造** —— `<租户ID>-x.io` 这个组里**只可能有该租户的对象**,别人的不在里面 |
| **B** | **原生**集群级资源的 **读**(webhook 配置、CRD…) | 授给一个**只命名单个 SA** 的组,kubezoo 转发时才断言 | **过滤** —— kubezoo 对集群级读按名字前缀过滤,`list` 与 `watch` 两条路都过 |

⛔ **B 为什么不能像 A 那样干净**:`resourceNames` 对 `list`/`watch` **不生效**(RBAC 的定义,
不是实现缺陷)⇒ 上游那条授权**必然是"全部"**,能看到什么完全由回程过滤决定。

⚠️ **B 的三条约束都是承重的**:

1. **组只命名一个 ServiceAccount**,不是租户。租户分 SA 就是为了分权;按租户发等于
   "它跑的每个容器都和它自己一样有权"。
2. **授的是交集** —— 该 SA 的 ClusterRole 里要的 ∩ 租户被允许的集群级集合。少了前者是过度授权;
   少了后者,租户给自己写一份更宽的 ClusterRole 就能经由自己的工作负载把差额收走。
3. **授在组上,不在 SA 上**。所以一个身份**从任何别的路径**到达上游都什么也拿不到 ——
   这也是为什么 `kubectl auth can-i --as=<该 SA>` 对这些资源**仍然答 no**,那不是 bug。

⭐ **写动词不走 B**:集群级对象按名字加租户前缀,`get`/`update`/`patch`/`delete` 都带名字
⇒ 构造上就够不到别人的,不需要额外机制。
...
> ## 一句话原则(用户定案)
> **租户看不到节点,没有任何调度权,他写的东西会被平台替换掉。**

这条是本节的**判据**,不是概括。它一次性收掉了几个原本要分头讨论的问题:
`schedulerName` 归谁 —— 平台替换的字段之一,不必单独讨论;
`nodeSelector`/`affinity` 该白名单还是该拒绝 —— 都不是,**替换**;
`topologySpreadConstraints` 租户能不能写 —— 不能,替换。

⚠️ 但把它当判据用,立刻暴露三处目前**还不满足它**的地方:

| 原则里的这半句 | 现状 |
|---|---|
| 「看不到节点」 | 部分成立且**已定案接受**:Node **对象**藏了 list/get/watch 三条路径,但**节点名**从每个 Pod 的 `spec.nodeName` 漏出。⇒ 承重的是后半句,不是这半句(§8.2.3) |
| 「写的东西会被替换」 | 设计已定,**尚未实现**;现有规则是**拒绝**不是替换,且上线注入时会打架(见下) |
| 「没有任何调度权」 | ⚠️ **替换发生在 Pod 准入,而 `pods/binding` 是另一次写入** —— 它在 Pod 建好之后直接落点,绕开替换(§Q,未实测) |

⇒ **覆盖面必须是"所有能影响落点的写入路径",不只是 PodSpec 字段。**
...
Node **对象**藏了三条读路径(§7.1),但**节点名**从每一个 Pod 的 `spec.nodeName`
漏给租户(实测,审计 §P);代码侧确认 `pkg/convert` / `pkg/proxy` / `pkg/util`
**一处都没碰过 `nodeName`**。

**定案(用户 2026-07-29):接受,不改。**
知道节点名**只在能拿它做事的时候才值钱**,而按本节原则落点字段全部被平台替换
⇒ 名字是**兑现不了的信息**。藏掉它则会让 `kubectl -o wide` / `describe pod` 失真,
**代价确定、收益为零**。

⚠️ **这条定案绑着一个条件**:替换必须覆盖到 `pods/binding` —— 它不是"设置",
是**直接写节点名的一次写入**(§Q)。兜住它的是 kubelet 会核对的那个注入 `nodeSelector`,
所以 **"注入标签每租户专属"是本定案的前提**,不是优化项。

⇒ 一句话原则里「看不到节点」这半句**不是承重件**,承重的是「没有调度权 + 写的会被替换」。
...
- 要区分两类破坏:**容器内的**(补 RoleBinding 一点用没有)和 **k8s 对象层的**
  (补了确实有用)。而后者的正解**不是把冻结做严,而是快照** ——
  取证要的是"某个时间点的对象视图",不是指望活集群一直保持原样。
  有了快照,之后 SA 再删**本身就是证据**(篡改行为有记录)

⛔ **但快照不在停机机制里做**(已定):一份快照要能说清它是**哪个时间点 / 哪个 revision**
的视图、怎么存、留多久、谁能读 —— 这些保证停机机制给不了,硬塞进来等于让它背上一个
兑现不了的承诺。**停机只管控制面的事**;快照归未来的**快照语义**路径统一处理。
⚠️ 那条路径本身还没设计,这里只是把边界划清,不是已规划的工作。

所以补 RoleBinding 是在给一个不该承担这个职责的机制加码。控制器仍然**每次 Frozen 都列出
这些绑定**,但那是**陈述边界供运维决策**,不是待办。

### 职责边界:kubezoo 只交付"控制面冻结"这个原语

⭐ **kubezoo 的产出到此为止**:把租户的操作能力冻住,并让这件事**可被编排层操作**
(Tenant CR 上的一个字段 + 可读的拒绝文案 + 上游 RBAC 收回)。

拿它去干什么 —— 要不要抓快照、要不要连数据面一起冻、节点上跑什么 agent、
取证流程怎么走 —— 都是**上层编排的决策**。控制面组件不该规划这些,
也不该在日志里指挥运维怎么做数据面。

### 数据面侧的已知手段(供编排层参考,**不属于 kubezoo**)

记在这里只是因为它推翻了一个容易犯的判断:"停住容器内的活动"**不必**杀 Pod。
...
真要用,顺序是**先摘探针(或改重启策略),再冻**。

⚠️ 这是**节点级、带外**的操作,需要节点侧特权组件。**kubezoo 碰不到 cgroup,也不打算碰。**
