...
| 引用平台的集群级对象(RuntimeClass / PriorityClass / IngressClass) | 拿到平台的运行时或全集群最高优先级 |
...
| 约束 | 现状 |
|---|---|
| `runtimeClassName` / `ingressClassName` / `priorityClassName` 由平台决定 | ⚠️ **过渡态暂放在 kubezoo**,策略上线即移除 |
