...
- [ ] ⛔ **P0:跨租户主机名抢占无人裁决**(审计 §AH,已实测)
      任何租户可声明任何 `host`,**先到先得,落败方零报错**(两个方向都验过)。
      kubezoo 表达不了这个维度(host 是自由 DNS 名,前缀化就毁了它的用途)。
      可选形态(均未实现):按租户分配子域并用策略校验 / 每租户独立 ingress 控制器 /
      准入层维护"主机名→租户"登记表
- [ ] ⚠️ **`ingressClassName` 只擦不注入 ⇒ 租户的 Ingress 现在是惰性的**(审计 §AH):
      策略擦掉 class,控制器就全部忽略。而策略**不看谁提交**,平台事后 patch 也会被擦 ——
      必须由策略**注入**。与落点那条(§8.2.2)是同一个形状
